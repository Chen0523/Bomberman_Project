...
class DQN(object):
	...
	def choose_action(self, self_mat, x,train = False):
		"""
		...
		if np.random.uniform() < EPSILON:  # choose an optimize function
			...
			action_np = actions_value[0].detach()
			action_np[self_mat == 0] = -999

			# print(action_np)
			max_v = np.argmax(action_np).data.numpy()

			# action_t = torch.max(actions_value, 1)[1].data.numpy()  # choose the action that returns max. reward
			# print(action_t)
			action = np.array([max_v])
		else:
...
class Net(nn.Module):
	def __init__ (self,in_channels:int,out_channels:int,kernel_size:int,action_space = 6,stride = 1,dropout_f = 0.01):
		...
		self.conv = nn.Sequential(
			...
			nn.ReLU(),
			nn.Conv2d(out_channels, out_channels, int(kernel_size), int(stride), 0),
			nn.ReLU(),
		)
	...
	def _get_conv_out(self):
		o = self.conv(torch.zeros(1,4,1,6))
